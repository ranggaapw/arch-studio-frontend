...
import {
  Award, ArrowRight, X, Sparkles, Wallet, Gem, Tag,
  User, Calendar, MapPin, BrickWall, ChevronDown
} from 'lucide-react';
...
const PRIORITY_PRESETS: Record<PriorityKey, AhpRequestType> = {
  ...
  kategori: { bahanVsBudget: 0.333, bahanVsKategori: 0.2, budgetVsKategori: 0.2 },
};
...
const PRIORITY_OPTIONS: {
  ...
  },
  {
    ...
    sublabel: 'Tampilkan proyek yang paling relevan dengan kategori desain Anda',
    icon: <Tag size={22} />,
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 md:p-8 overflow-y-auto">
      ...
      >
        ...
        <div className="w-full md:w-[60%] p-6 md:p-8 flex flex-col justify-between bg-neutral-50/50 border-r border-neutral-100 max-h-[50vh] md:max-h-full">
          <div className="flex-1 relative rounded-2xl overflow-hidden bg-neutral-100 h-[250px] md:h-[450px] lg:h-[500px]">
            <motion.img
              key={activeIdx}
              initial={{ opacity: 0.5 }}
              animate={{ opacity: 1 }}
            ...
            <div className="flex gap-2.5 mt-4 overflow-x-auto scrollbar-hide py-1.5">
              {modalImages.map((img, idx) => (
                <button
                  ...
                  className={`relative w-20 h-16 rounded-xl overflow-hidden flex-shrink-0 cursor-pointer transition-all ${
                    activeIdx === idx
          ...
          <div>
            <div className="flex flex-wrap gap-1.5 mb-3.5">
              {item.categories?.map((cat) => (
                <span key={cat} className="px-3 py-1 bg-primary-50 text-primary-600 rounded-full text-body-sm-bold uppercase tracking-wider">
                  {cat}
                </span>
              ))}
            </div>
            <h2 className="text-2xl font-bold text-neutral-900 mb-5 leading-tight">{item.title}</h2>
            <div className="grid grid-cols-2 gap-4 border-y border-neutral-100 py-4 mb-5 text-sm text-neutral-600">
              <div className="flex items-start gap-2.5">
                <div className="p-1.5 bg-neutral-50 rounded-lg text-primary-600"><User size={15} /></div>
                <div>
                  <span className="text-[10px] text-neutral-400 block uppercase tracking-wider mb-0.5">Klien</span>
                  <span className="text-neutral-800 font-semibold">{item.clientName || 'N/A'}</span>
                </div>
              </div>
              <div className="flex items-start gap-2.5">
                <div className="p-1.5 bg-neutral-50 rounded-lg text-primary-600"><Calendar size={15} /></div>
                <div>
                  <span className="text-[10px] text-neutral-400 block uppercase tracking-wider mb-0.5">Tahun</span>
                  <span className="text-neutral-800 font-semibold">{item.year || 'N/A'}</span>
                </div>
              </div>
              <div className="flex items-start gap-2.5 col-span-2">
                <div className="p-1.5 bg-neutral-50 rounded-lg text-primary-600"><MapPin size={15} /></div>
                <div>
                  <span className="text-[10px] text-neutral-400 block uppercase tracking-wider mb-0.5">Lokasi</span>
                  <span className="text-neutral-800 font-semibold">{item.location || 'N/A'}</span>
                </div>
              </div>
            </div>
            <div className="mb-5">
            ...
            <div className="mb-6">
              <span className="text-neutral-400 text-xs uppercase tracking-wider font-bold block mb-1.5">Deskripsi Proyek</span>
              <p className="text-sm text-neutral-600 leading-relaxed max-h-[160px] overflow-y-auto pr-2">
                {item.description}
              </p>
            </div>
          </div>
...
function ProjectRankCard({
  ...
  return (
    ...
    >
      {isTop && (
        <div className="absolute top-3.5 left-3.5 z-10 bg-gradient-to-r from-amber-400 via-yellow-400 to-amber-500 text-neutral-900 text-[10px] font-extrabold uppercase tracking-wider px-3 py-1.5 rounded-full flex items-center gap-1.5 shadow-md border border-amber-300">
          <Award size={11} /> Rekomendasi Terbaik #1
        </div>
      )}

      {/* Thumbnail */}
      <div className={`w-full sm:w-[200px] h-[160px] sm:h-auto flex-shrink-0 overflow-hidden ${isTop ? 'sm:rounded-xl sm:m-2' : ''}`}>
      ...
      <div className="p-5 flex flex-col justify-between flex-grow">
        <div>
          ...
          <h4 className="font-bold text-neutral-900 text-base leading-snug mb-1.5">{item.title}</h4>
          <p className="text-xs text-neutral-400 line-clamp-2 leading-relaxed">{item.description}</p>
        </div>
        <div className="flex items-center justify-between mt-4">
          <span className="text-[10px] text-neutral-400 font-semibold">Peringkat #{index + 1}</span>
          <button
          ...
          >
            Lihat Detail <ArrowRight size={13} className="ml-1 group-hover:ml-2 transition-all" />
          </button>
        </div>
      </div>
    </motion.div>
...
export default function BestDesignSection() {
  ...
  // Mutasi AHP
  const { mutate, data: ahpResp, isPending, isSuccess } = useMutation({
  ...
  const visibleRankings = showAll ? displayRankings : displayRankings.slice(0, 3);
  ...
  return (
    <section className="w-full bg-neutral-50 py-20 px-6 md:px-8 border-t border-neutral-100">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
          <div>
            ...
            <p className="text-neutral-400 mt-3 max-w-lg leading-relaxed text-sm">
              Pilih prioritas Anda — sistem kami akan otomatis mengurutkan proyek terbaik menggunakan metode AHP.
            </p>
          </div>
            ...
            return (
              ...
              >
                ...
                <div className={`mt-0.5 w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-colors ${
                  isActive ? 'border-primary-600 bg-primary-600' : 'border-neutral-300'
                }`}>
                ...
                <div>
                  {/* Icon + Label */}
                  <div className={`flex items-center gap-2 font-bold text-sm mb-1 ${isActive ? opt.activeText : 'text-neutral-700'}`}>
                    <span className={`${isActive ? opt.activeText : 'text-neutral-400'} transition-colors`}>
                      {opt.icon}
                    </span>
                    {opt.label}
                  </div>
                  <p className="text-xs text-neutral-400 leading-relaxed">{opt.sublabel}</p>
                </div>
              </button>
            );
          })}
        ...
        {/* ── Active label strip ── */}
        <div className={`flex items-center gap-2 text-xs font-bold uppercase tracking-widest mb-6 ${activeOption.activeText}`}>
          <div className={`w-6 h-0.5 bg-gradient-to-r ${activeOption.gradient}`} />
          Diurutkan berdasarkan: {activeOption.label}
          <div className={`flex-grow h-0.5 bg-gradient-to-r ${activeOption.gradient} opacity-20`} />
        </div>

        {/* ── Kartu Hasil ── */}
        <div className="flex flex-col gap-5">
          {/* Loading skeleton */}
          {isPending && displayRankings.length === 0 &&
            [1, 2, 3].map((i) => (
              <div key={i} className="bg-white rounded-2xl p-5 border border-neutral-100 flex gap-5 items-center animate-pulse">
                <div className="w-[200px] h-[120px] bg-neutral-100 rounded-xl flex-shrink-0" />
        ...
        {displayRankings.length > 3 && (
          <div className="flex justify-center mt-8">
            ...
            >
              {showAll ? 'Tampilkan Lebih Sedikit' : `Lihat Semua ${displayRankings.length} Proyek`}
              <ChevronDown
                size={16}
